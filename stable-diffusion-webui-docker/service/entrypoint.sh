...
mkdir -p /data/models/VAE-approx/ /data/models/karlo/ /data/models/Stable-diffusion /data/models/VAE
...
MOUNTS["${ROOT}/dependencies"]="/data/dependencies"
...
# Download models
ls "${ROOT}/embeddings"
while read -r target_file file_url; do
      if [ ! -f "${ROOT}/$target_file" ]; then
          wget -O "${ROOT}/$target_file" "$file_url"
          echo "File downloaded: ${ROOT}/$target_file"
      else
          echo "File already exists: ${ROOT}/$target_file"
      fi
done < "${ROOT}/dependencies/links.txt"
